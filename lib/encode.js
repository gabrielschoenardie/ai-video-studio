...
const { measure } = require('./vmaf');
const { buildGrade, buildCaptions, describe } = require('./color');
const { hasZscale } = require('./deps');
...
// `sourceKind === 'mezzanine'` skips the chroma and bitrate rules below:
// from Task 4 onward the mezzanine is 4:4:4 high-bitrate by design, so
// flagging it as risk there is a false positive.
function riskScore(info, sourceKind = 'external') {
  ...
  add(info.colorRange === 'pc', 1, 'source full-range (PC) color — remapped to broadcast (TV) range');
  add(info.fps > 50, 1, `source ${(info.fps || 0).toFixed(1)} fps (60p needs conversion)`);
...
//
// `tag` suffixes the internal [bg]/[fg] labels of the 'blur' branch. It exists
// for -filter_complex graphs that fit more than one stream (lib/timeline.js
// fits the base plus every B-ROLL clip): identical labels in two chains of the
// same graph are a "duplicate output label" error. Default '' keeps the string
// byte-identical for the single-stream -vf callers (encode.js, assemble.js).
const FIT_W = 1080, FIT_H = 1920;
function buildFit(fit, sopt, tag = '') {
...
// Canonical vf order (do not reorder — see docs/plans/color-pipeline-order-and-dither.md):
//   1. denoise            (optional, YUV domain, source resolution)
//   2. buildFit(fit)      → 1080×1920, aspect-preserving, in_range/out_range
//   3. fps=30
//   4-5. GRADE + DITHER   → 3D LUT (plate only) + RPDF dither, via lib/color.js
//   6. subtitles(.ass)    ← graphics enter here, after the LUT, ungraded
//   7. format=yuv420p     ← the pipeline's single chroma subsampling
function buildArgs(input, output, profile, { lut = null, denoise = null, x264 = {}, fullRangeSource = false,
  fit = 'blur', captions = null, grade = 'plate', dither = 'random', zscale = false } = {}) {
  const vf = [];
  if (denoise) vf.push(denoise);                                   // 1 — derived upstream, never frei0r=bm3d
  ...
  const gradeLut = grade === 'none' ? null : lut;                   // 'none' ignores the LUT even if set
  vf.push(...buildGrade({ lut: gradeLut, dither, zscale }));        // 4 + 5 — LUT touches the plate only
  vf.push(...buildCaptions(captions));                              // 6 — graphics, after the grade
  vf.push('format=yuv420p');                                        // 7 — single chroma subsampling
...
async function validate(output) {
  ...
  chk('color_range tv', v.color_range === 'tv', v.color_range || 'untagged');
...
// ------------------------------------------------------------- pipeline
async function encodeReel(input, output, { lut = null, denoise = null, x264 = {}, fit = 'blur',
  captions = null, grade = 'plate', dither = 'random',
  sourceKind = 'external', reference = null, onProgress, onLog } = {}) {
  ...
  const risk = riskScore(info, sourceKind);

  const zscale = await hasZscale();
  if (lut && !zscale) onLog && onLog('[color] zscale ausente — LUT em 8-bit, dither por noise t+u\n');

  const args = buildArgs(input, output, profile, {
    lut, denoise, x264, fullRangeSource: info.colorRange === 'pc', fit,
    captions, grade, dither, zscale,
  });
  ...
  try {
    ...
    // The reference must receive the same creative transformations (grade +
    // captions) as the distorted output, in high precision, uncompressed —
    // otherwise the LUT/legend show up as distortion and the score is
    // meaningless. See docs/plans/color-pipeline-order-and-dither.md.
    const refFilter = [
  ...
  return {
    profile, risk, validation, vmaf,
    color: describe({ lut: grade === 'none' ? null : lut, dither, zscale, captions }),
    command: 'ffmpeg -hide_banner -y ' + args.map(a => /[\s'"]/g.test(a) ? JSON.stringify(a) : a).join(' '),
    sourceInfo: info,
  };
}

module.exports = { selectProfile, riskScore, buildArgs, buildX264Params, validate, encodeReel, buildFit };
