...
function buildArgs(input, output, profile, { lut = null, denoise = null, x264 = {}, fullRangeSource = false, fit = 'blur' } = {}) {
  ...
  // ffmpeg filtergraph path escaping — on Windows the backslashes and the
  // drive-letter colon break the option parser (lut3d reads a mangled path);
  // same recipe captions.js subFilter uses: '\'→'/', ':'→'\:', "'"→"\'".
  if (lut) {
    const esc = lut.replace(/\\/g, '/').replace(/:/g, '\\:').replace(/'/g, "\\'");
    vf.push(`lut3d='${esc}'`);
  }
