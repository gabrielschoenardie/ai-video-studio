...
// stdout to the console's codepage (cp1252), which can't encode some
// whisper output (e.g. detected-language names in non-Latin scripts).
...
async function viaOpenaiWhisper(wav, workDir, model, language, onLog) {
  ...
  return { text: (j.text || '').trim(), words: mergeSymbolTokens(words), segments: (j.segments || []).map(s => ({
...
async function viaWhisperCpp(wav, workDir, model, language, onLog) {
  ...
  return { text: segments.map(s => s.text).join(' '), words: mergeSymbolTokens(words), segments };
}
...
module.exports = { transcribe, mergeSymbolTokens };
