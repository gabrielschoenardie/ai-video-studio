...
const { hasLibvmaf } = require('./vmaf');

// Same pattern as hasLibvmaf (lib/vmaf.js): probe `ffmpeg -filters` for the
// filter name, never reject, degrade to false on any error.
  ...
  return new Promise((resolve) => {
    execFile('ffmpeg', ['-hide_banner', '-filters'],
      ...
      (err, stdout) => {
        ...
        resolve((stdout || '').includes('zscale'));
      });
  });
}

// PYTHONIOENCODING=utf-8 avoids a crash on Windows: Python defaults its
// stdout to the console's codepage (cp1252), and whisper --help output
// includes non-Latin-1 characters (e.g. CJK language names).
...
async function detect() {
  const [ffmpeg, ffprobe, ytdlp, whisper, whisperCpp, voicebox, python, libvmafOk, zscaleOk] =
    await Promise.all([
      ...
      probeVoicebox(),                       // Voicebox local server (voiceover engine)
      probe('python3', ['--version']),
      hasLibvmaf(),                          // libvmaf filter compiled into ffmpeg
      hasZscale(),                           // zscale filter compiled into ffmpeg (libzimg)
    ]);
...
module.exports = { detect, hasZscale };
