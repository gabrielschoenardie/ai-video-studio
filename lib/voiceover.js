...
// its local REST API. Fallbacks: VoxCPM, piper, espeak-ng, macOS `say` — so
// the pipeline still produces a scratch narration on any machine.
'use strict';
...
async function generate(script, outDir, { refVoice = null, voice = null, onLog } = {}) {
  ...
  // 2) VoxCPM CLI
  attempts.push({ name: 'voxcpm', fn: async () => {
  ...
  // 4) espeak-ng (robotic, but proves the pipeline)
